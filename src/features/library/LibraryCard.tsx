...
import { Star, Clock, CheckCircle2, PlayCircle, Tv, Book, Play } from "lucide-react";
...
const getStatusColor = (status: MediaItem["status"]) => {
  switch (status) {
    ...
    default:
      return "from-slate-500/30 to-slate-600/20 text-slate-300 border-slate-500/30";
  }
};
...
export default function LibraryCard({ item, onClick }: Props) {
  ...
  return (
    <div
      onClick={onClick}
      className="group relative bg-slate-800/30 backdrop-blur-md rounded-2xl overflow-hidden border border-white/5 hover:border-purple-500/40 transition-all duration-300 hover:shadow-2xl hover:shadow-purple-500/10 cursor-pointer flex flex-col"
    >
      ...
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 backdrop-blur-md ${
            isAnime ? "bg-blue-500/80 text-white" : "bg-pink-500/80 text-white"
          }`}>
        ...
        <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-full bg-black/60 backdrop-blur-md">
          ...
          <span className="text-white text-xs font-bold">{item.rating ? Number(item.rating).toFixed(1) : "0.0"}</span>
        </div>
        
        {/* Status Badge */}
        <div className="absolute bottom-3 left-3">
          <span className={`px-3 py-1 rounded-full text-xs font-semibold border bg-gradient-to-br ${getStatusColor(item.status)} flex items-center gap-1.5`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="w-12 h-12 rounded-full bg-purple-600/80 backdrop-blur-md flex items-center justify-center border border-white/30 hover:scale-110 transition-transform shadow-lg shadow-purple-500/30">
      ...
      <div className="p-4 flex flex-col flex-1 justify-between">
        <div>
          <h4 className="font-bold text-white text-sm md:text-base mb-1 truncate group-hover:text-purple-300 transition-colors" title={item.title}>
            {item.title}
          </h4>
          ...
          {item.notes && (
            <p className="text-slate-400 text-xs line-clamp-2 my-1">{item.notes}</p>
          )}
        </div>
        
        {/* Meta Info */}
        <div className="flex items-center justify-between text-xs text-slate-500 pt-2.5 border-t border-white/5 mt-2">
          <span>{isAnime ? `${item.episodes || 0} eps` : `${item.chapters || 0} chs`}</span>
          <span className="text-purple-400 font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
            Edit Details →
          </span>
        </div>
      </div>
