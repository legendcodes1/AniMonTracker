...
const Library: React.FC = () => {
  ...
  const stats = {
    ...
    avgRating:
      collection.length > 0
        ? (
            collection.reduce((acc, i) => acc + (i.rating || 0), 0) /
            collection.length
          ).toFixed(1)
        : "0.0",
  };

  const continueWatching = collection
                    ...
                    <div className="absolute bottom-2 left-2 right-2">
                      <div className="w-full bg-slate-700 rounded-full h-1 mb-1">
                        <div
                          ...
                          style={{
                            width: `${item.episodes && item.currentEpisode ? (item.currentEpisode / item.episodes) * 100 : 0}%`,
                          }}
                        />
                      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-500 text-sm mt-2 mb-6">
              Start adding anime and manga to build your collection
            </p>
            <button
              onClick={() => {
                setModalItem(null);
